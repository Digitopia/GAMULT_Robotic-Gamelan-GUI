...
void teste  () {
  for(int i=0; i<=myModules[i].population.size()-1; i++){
    print ("myModules" + " " + i);
    myModules[i].population.remove(i);
    
    //myModules[i].removeParticle(i);
} 
  increment = 0;

}

// tecla 'R' = Inicializa o sistema com 0 eventos (bolas)
void keyPressed () {
  if (key == 'r' || key== 'R') {
     for (int i=0; i<=myModules.length-1; i++) {
    myModules[i].eraseParticle();
     }
  }
  
  if (key == '+'){
    for(int i=0; i<=myModules.length-1; i++){
      myModules[i].console.gravity.keySupport(true); //Suporte de teclado para o Gravity de todos os módulos;
    }
  }
  
  if (key == '-'){
    for(int i=0; i<=myModules.length-1; i++){
      myModules[i].console.gravity.keySupport(false); //Idem 
    }
  }
  
  if (key == 'f' || key== 'F'){
    for(int i=0; i<=myModules.length-1; i++){
      myModules[i].console.freeze.keySupport(); //Suporte de teclado para o Freeze de todos os módulos;
    }
  }
